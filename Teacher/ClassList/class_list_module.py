...
class ClassListModuleWindow(Frame):
    def __init__(self, master):
        ...
        self.columns = ('date', 'name', 'code', 'size')
        self.headings = ('Date', 'Class Name', 'Class Code', 'Class Size')
    ...
    def search_command(self, event):
        try:
            ...
            for data in A_S_M_S_D.tea_t_data(self.search_entry.get().title()):
                ...
                for cls in A_S_M_S_D.tea_c_list(data[12]):
                    self.display_tree.insert('', END, cls[0], text=cls[0])
                    self.display_tree.set(cls[0], self.columns[0], cls[1])
                    self.display_tree.set(cls[0], self.columns[1], cls[2])
                    self.display_tree.set(cls[0], self.columns[2], cls[3])
                    self.display_tree.set(cls[0], self.columns[3], cls[6])
        except Exception:
